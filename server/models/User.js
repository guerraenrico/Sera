...
const New = (
  googleId, email, name, locale, pictureUrl,
  refreshToken = undefined, createdAt = undefined, updatedAt = undefined,
  id = undefined,
) => (
  {
    ...
    ...((refreshToken !== undefined) && { [Schema.fields.refreshToken]: refreshToken }),
...
const CreateFromDocument = userDocument => New(
...
const GetAsync = async (db, id) => {
  const userDoc = await db.collection(Schema.name).findOne({ _id: ObjectId(id.toString()) });
...
const GetByGoogleIdAsync = async (db, googleId) => {
  const userDoc = await db.collection(Schema.name).findOne({ [Schema.fields.googleId]: googleId });
  if (userDoc === undefined || userDoc === null) {
    return undefined;
  }
  return CreateFromDocument(userDoc);
